...
const TIER_OFFERS: Record<string, { title: string; subtitle: string; icon: string; color: string; bg: string; border: string }> = {
  bronze:   { title: "Starter Picks for You",       subtitle: "Great value items to kickstart your fan journey",                        icon: "🥉", color: "#cd7f32", bg: "rgba(205,127,50,0.08)",  border: "rgba(205,127,50,0.25)"  },
  silver:   { title: "Silver Fan Picks",             subtitle: "Rare items and boosters matched to your Silver status",                  icon: "🥈", color: "#94a3b8", bg: "rgba(148,163,184,0.08)", border: "rgba(148,163,184,0.25)" },
  gold:     { title: "Gold Exclusives for You",      subtitle: "Premium selections curated for Gold Fans — 10% coin bonus active",      icon: "🥇", color: "#fbbf24", bg: "rgba(251,191,36,0.08)",  border: "rgba(251,191,36,0.28)"  },
  platinum: { title: "Platinum Specials",            subtitle: "Epic & Legendary items at your 20% platinum discount",                   icon: "💎", color: "#38bdf8", bg: "rgba(56,189,248,0.08)",  border: "rgba(56,189,248,0.25)"  },
  diamond:  { title: "Diamond-Only Selections",      subtitle: "Exclusive items and drops available only to Diamond Fans",               icon: "👑", color: "#a78bfa", bg: "rgba(167,139,250,0.1)",  border: "rgba(167,139,250,0.3)"  },
  legend:   { title: "Legend Tier — All Access",     subtitle: "Every item, every drop — you have maximum access",                       icon: "🔥", color: "#ef3976", bg: "rgba(239,57,118,0.08)",  border: "rgba(239,57,118,0.3)"   },
};
...
// ─── Categories ───────────────────────────────────────────────────────────────
const CATEGORIES: { id: ShopCategory | "mystery_boxes" | "for_you" | "inventory"; label: string; icon: string }[] = [
...
// Non-shop categories that should never render shop item grids
const NON_SHOP_CATS = ["mystery_boxes", "for_you", "inventory"];
      ...
      <div className="w-full max-w-sm rounded-[22px] border overflow-hidden"
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div className="flex gap-2.5">
            <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl text-[12px] font-black border"
            ...
            <button onClick={onConfirm} disabled={!canAfford || isPurchasing}
              className="flex-[2] py-2.5 rounded-xl text-[12px] font-black text-white flex items-center justify-center gap-2"
              ...
              {isPurchasing
                ? <><svg className="animate-spin size-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>Buying…</>
...
export default function ShopTab({ statusXp = 0 }: ShopTabProps) {
  ...
  useEffect(() => {
    if (lastPurchase) {
      ...
      setInvRefreshKey((k) => k + 1); // refresh inventory after any purchase
    }
  }, [lastPurchase]);
  ...
  const handleConfirm = useCallback(async () => {
    ...
    await purchase(selectedItem.id, "coins");
  }, [selectedItem, purchase]);

  const handleCategoryClick = (catId: string) => {
    setActiveCategory(catId);
    if (catId === "inventory") setInvRefreshKey((k) => k + 1);
  };

  // Filter shop items — inventory tab has its own data source so exclude it here
  const filtered =
  ...
  // Is the current tab a non-shop view?
  const isInventoryTab   = activeCategory === "inventory";
  const isMysteryTab     = activeCategory === "mystery_boxes";
  const isForYouTab      = activeCategory === "for_you";
  const isRegularShopTab = !NON_SHOP_CATS.includes(activeCategory);
  ...
  return (
    <div className="flex flex-col gap-6 pb-8" style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {CATEGORIES.map((cat) => {
          const count =
            cat.id === "all"       ? items.length
            : cat.id === "for_you" ? items.filter((i) => allowedRarity.includes(i.rarity)).length
            : cat.id === "inventory" ? 0  // inventory has its own count inside InventoryTab
      ...
      {/* ── Inventory tab — completely independent, no isLoading gate ── */}
      {isInventoryTab && (
        <InventoryTab key={invRefreshKey} />
      )}

      {/* ── Shop content — hidden when on inventory tab ── */}
      {!isInventoryTab && (
        isLoading ? (
          ...
          <>
                ...
                {tier.id !== "legend" && (
                  <div className="rounded-[18px] border p-4 flex items-center gap-4"
                    ...
                    <div className="flex-1">
                      <p className="text-[12px] font-black" style={{ color: TEXT }}>Rank up to unlock more items</p>
                      <p className="text-[11px] mt-0.5" style={{ color: MUTED }}>Higher status unlocks exclusive offers, discounts and rare drops</p>
                    </div>
                  </div>
                )}
              </section>
            ...
            {/* Mystery boxes */}
            {(activeCategory === "all" || isMysteryTab) && (
              <section>
            ...
            {/* Featured items — regular shop tabs only */}
            {featured.length > 0 && isRegularShopTab && (
              <section>
            ...
            {/* Regular items */}
            {regular.length > 0 && isRegularShopTab && (
              <section>
            ...
            {/* Empty state — only for regular shop tabs, not mystery/for_you */}
            {filtered.length === 0 && isRegularShopTab && (
      ...
      <div className="flex items-center gap-4 rounded-[16px] border px-5 py-4"
        ...
        <button
            onClick={() => router.push("/dashboard/user/fan-pass?tab=quests")}
            className="flex-shrink-0 rounded-xl px-4 py-2 text-[11px] font-black text-white transition-all hover:opacity-90"
